...
moveObject computer obj =
  { obj
    | x = obj.x + obj.dx
    , y = obj.y + obj.dy
    , dx = bounce (obj.x + obj.dx - largeAsteroidRadius < computer.screen.left) (obj.x + obj.dx + largeAsteroidRadius > computer.screen.right) obj.dx
    , dy = bounce (obj.y + obj.dy - largeAsteroidRadius< computer.screen.bottom) (obj.y + obj.dy + largeAsteroidRadius > computer.screen.top ) obj.dy
    , dir = obj.dir + obj.spin
  }

-- if (obj.x + obj.dx - 2*largeAsteroidRadius < computer.screen.left || obj.x + obj.dx + 2*largeAsteroidRadius > computer.screen.right ) then obj.dx * (-1) else obj.dx
...
-- this is a helper function written by Julia Kispert to have the asteroids bounce off the edges of the screen 
bounce min max dx = 
  if (min || max) 
  then 
    dx * (-1) 
